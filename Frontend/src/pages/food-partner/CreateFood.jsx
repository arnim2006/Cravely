...
const CreateFood = () => {
    ...
    const onSubmit = async (e) => {
        ...
        try {
            ...
            setUploadError(err.response?.data?.error || err.response?.data?.message || 'Failed to upload video to ImageKit. Check server logs.');
        } finally {
        ...
        <div className="create-food-page">
            <div className="create-food-card">
                ...
                <form className="create-food-form" onSubmit={onSubmit}>
                    ...
                    {uploadError && <p className="error-text" role="alert" style={{color: 'var(--color-danger, #ef4444)', fontSize: '0.9rem', marginBottom: '15px', textAlign: 'center'}}>{uploadError}</p>}
                    <div className="form-actions">
                        <button className="btn-primary" type="submit" disabled={isDisabled || isUploading}>
                            {isUploading ? 'Uploading to ImageKit...' : 'Save Food'}
                        </button>
                    </div>
                </form>
            </div>
